...
6. 🟡 **Consumers (incremental; legacy `actum.stage` shim stays until 6e):** `StatusView`/bulletin/frontend read `Progressus`; derive `ActumExecutio` from phase durations.
   ...
   - 🟠 6b bulletin reads `actum.progressus`; 6c `RunEventHub`/SSE emit `Progressus`; 6d derive `ActumExecutio` from `phaseDurations`; 6e delete `progressusToStage` + `actum.stage` + `emitStage`.
