...
| `running` + "Generating baseline" (step 0) | `warming` | pre-train sample = readiness inference, not a VRAM load |
...
5. 🟡 **Training (projector LANDED + ground-truth-validated 2026-06-22; runner pending):** crystal-native — drive **ostris/ai-toolkit** directly and read its typed SQLite `Job` row, retiring the legacy `TrainingOutputParser` regex slop entirely (decided: no legacy VastAI wrap). The pure §6c projector `aitkJobToProgressus(job, cfgSteps)` (`src/execution/aitkProgressus.ts`) maps ai-toolkit's two-axis `status`×`info` job state → `Progressus`, including `executing {done:step,total,steps}` + `etaMs` parsed from `speed_string` (both `iter/sec` and `sec/iter`), and is coalescing-aware (steady "Training" pings carry no `message`, so ticks stay live-only §7). **Validated against real ground truth:** a local **FLUX.2 Klein-4B** smoke (60 steps, the `stationthis` style LoRA on the 4090, `ui_trainer` → SQLite `Job` rows) ran end-to-end; the actual emitted timeline (`Starting`→`Loading Qwen3`→`Quantizing Qwen3`→`Loading model`→`Loading dataset`→`Generating baseline`→`Training`→`Training completed`) projects to a clean `loading → downloading → warming → executing → done` shape — the projector handled klein-specific load substates it never hardcoded (Qwen3 TE load/quantize via the `step≤0` loading floor; the pre-train baseline sample mapped to `warming`). Orchestration (image, config, launcher, preflight, runbook) lives with the trainer host, outside this repo. Tests: `tests/unit/crystal/aitkProgressus.test.ts` (15, incl. the captured ground-truth timeline). **Runner core LANDED:** `aitoolkitRunnerClient.awaitViaPoll` (`src/crystal/aitoolkitRunnerClient.ts`) — the poll loop (mirrors comfyrunner's `awaitViaStream`): reads ai-toolkit's `Job` row via an injected `AitkJobReader`, projects each read through `aitkJobToProgressus`, and records the timeline onto the run's training **Actum** via the same `recordProgressus`/`getTrace().actumId` seam — so unlike TEE the full timeline + `phaseDurations` persist. Records only on a changed `(status,step,info)` signature (matches ai-toolkit's own `poll_job` dedup; persistence further coalesced §7); returns a terminal `AitkOutcome` (never throws on a training failure — the cursor maps complete-vs-fail); a hung job hits `timeoutMs` and closes the timeline with a synthetic `failed`. Tests: `tests/unit/crystal/aitoolkitRunnerClient.test.ts` (7, scripted reader — no DB/GPU). **Live shell LANDED:** `SqliteAitkJobStore` (`src/crystal/AitkJobStore.ts` — built-in `node:sqlite`, no new dep; `seed` mirrors `seed_job.py`'s upsert, `read` is the `AitkJobReader` over `aitk_db.db`), `DockerAitkSpawner` (`src/crystal/AitkSpawner.ts` — `docker run -d` the ai-toolkit image, mirrors `train_stationthis.sh`), and `AitoolkitTrainingCursor` (`src/crystal/AitoolkitTrainingCursor.ts`, ministerium `'aitoolkit'`): `run()` seeds → spawns → `awaitViaPoll` to terminal recording the timeline onto the Actum → maps `completed` to exitus / `error`·`stopped` throw (Actum `fractus`, signa released). Self-hosted ⇒ reserves/charges 0n (host convention). Sync (a local box has no completion webhook; a remote/queued variant would go async). Seeded `Fundamentum FUNDAMENTUM_AITOOLKIT_TRAINING` (runtime `'aitoolkit'`, klein-4b self-download); registered in `container.ts` gated on a `config.aitoolkit` (dbPath/image/mounts) — absent in prod (no local trainer) ⇒ no registration. Tests: `AitkJobStore.test.ts` (3, real temp DB incl. a second-connection UITrainer UPDATE), `AitoolkitTrainingCursor.test.ts` (5, fake store+spawner). **Remaining = one live-verify:** an actual `train_stationthis.sh`-class run dispatched through a crystal training Actum (the `DockerAitkSpawner` docker invocation is the only piece CI can't exercise — same status as TEE's `runner.py`).
6. ✅ **Consumers (COMPLETE 2026-06-23):** `StatusView`/bulletin/SSE read `Progressus`; `ActumExecutio` durations derive from phase durations; the legacy `actum.stage` shim is fully retired (6e) — `actum.progressus` is the single status channel.
   - ✅ **6a (LANDED 2026-06-23):** cold-start records `Progressus` (closes the #3 scope boundary — a cold run's timeline now opens at `provisioning`, not `downloading`). New pure `coldStartProgressus(stage, info)` (`src/execution/progressus.ts`) maps the pod-lifecycle vocabulary ONLY (`provisioning`→`provisioning`, `pod-locked`/`warm-pod-found`→`provisioning` + the new `Progressus.pod` {podId,gpuType,region,costPerHr} + a persisting message, `bootstrapping`/`comfy-ready`→`pulling/fundamentum`); returns `undefined` for comfyrunner's own stages so there's no double-record. `SecurePodClient`'s two `emitStage` helpers + `WarmPodClient` now `recordProgressus` alongside the **untouched** `actum.stage` emit (shim parallel-runs). `Progressus.pod` added (identity/cost, first-class — VRAM stays on `resources`); `normalizeProgressus` parses it. Tests: progressusSink.test.ts (+5). `ActumExecutio` fold deferred to 6d.
   - ✅ **6a follow-up (LANDED 2026-06-23):** the three cold-start `recordProgressus` calls were fire-and-forget `void` from sync `emitStage` callbacks — a recorder DB error would surface as an unhandled rejection. `.catch(log.warn)` added (status must never break a run, §4).
   - ✅ **6b (LANDED 2026-06-23):** the Telegram bulletin is driven by the owned `actum.progressus` (single source). New `PodSession.onProgressus(p)` maps the `Phasis` vocabulary to the SAME journal/live transitions `onStage` made from the legacy strings (discriminators: `provisioning` ±`pod` = hunt vs lock; `pulling`+`runtime ready` = `comfy-ready`; warm reuse `message:'warm pod reused'` stays 🔥-only, never journaled; `loading`/`warming`/terminals = no bulletin line — completion is the WideEvent path). `BulletinManager.onProgressus` + a `_pendingProgressus` pre-register replay buffer mirror the `onStage` path. `TelegramAllocutio` subscribes `actum.progressus` → bulletin; `_handleActumStage` is trimmed to the 🔥/👌 reaction only (no double-drive). `FakeRunPodClient`/`FakeWarmPodClient` now record `Progressus` too (shared `fakeStageToProgressus`, covering the full fake vocabulary since the fake replaces comfyrunner) so dev mode tracks the single source. The `actum.stage` shim still emits in parallel (6c/SSE not yet migrated). **Retired:** the Fake/dev-only `pod-bailed`→"Quit pod N" line (the real throttle path re-provisions — a fresh hunt → Found — and never emitted a Quit); synthetic `phaseMs` display durations give way to real-elapsed timing. Tests: PodSession.test.ts (+4), BulletinManager.test.ts (+2), TelegramAllocutio.test.ts migrated to `actum.progressus`.
   - ✅ **6c (LANDED 2026-06-23):** `RunEventHub`/SSE stream the owned typed `Progressus`. New `RunEvent` kind `'progress'` (`busToRunEvent` projects `actum.progressus` → `{kind:'progress', terminal:false, progressus}`; drops a payload with no/invalid `progressus`); the hub subscribes `actum.progressus` alongside the existing three. **Never terminal** — cost + completion still ride `actum.complete`/`actum.fail` (the legacy `stage` frames keep flowing in parallel until 6e). The SSE endpoint (`GET /v1/runs/:id/stream`) already JSON-serialises any `RunEvent` (Progressus `at:Date` → ISO string) so no router change beyond the contract-summary line. Frontend wiring (`screens/Run.tsx`, mock) is the USER's seam — unchanged. Tests: runEvents.test.ts (+2), RunEventHub.test.ts (+1).
   - ✅ **6d (LANDED 2026-06-23):** the DURATION subset of `ActumExecutio` (`provisionMs`/`downloadMs`/`executionMs`) unifies into `phaseDurations` (§7). New pure `executioFromPhaseDurations(pd)` (`src/execution/progressus.ts`) sums each phase's dwell across targets (`provisioning`→provisionMs, every `downloading/*`→one downloadMs, `executing`→executionMs); identity/cost fields (podId/gpuType/costPerHr/coldStart/modelsDownloaded/billedMs/…) are NOT durations and stay cursor-sourced. Consumers **prefer the explicit cursor field, fall back to the derived** — so comfyrunner's precise self-reported inference time stays authoritative while a timeline-only runner (TEE/training) now lands these timings. Wired into `buildWideEvent` (`src/lib/wide.ts`) + `DeliveryView.formatStats`. **Cost is untouched** — it rides `billedMs`/`costPerHr`, never `executionMs`. Tests: progressus.test.ts (+4), wide.test.ts (+3, incl. explicit-wins + cost-unchanged guards).
   - ✅ **6e (LANDED 2026-06-23):** the stringly `actum.stage` shim is **fully retired** — `actum.progressus` is now the single status channel end-to-end. Deleted: `progressusToStage`; the `actum.stage` bus event (`bus.ts`); every emitter (`SecurePodClient`'s `emitStage`→`signal`, `WarmPodClient`, `comfyrunnerClient`'s `emitStage` param + the per-tick `progress`→`actum.stage` SSE frame, `FakeRunPodClient`/`FakeWarmPodClient`); `PodSession.onStage` (+ orphaned `_phaseMs`/`_bail`); `BulletinManager.onStage` + `_pendingStages`; the `'stage'` `RunEvent` kind + `busToRunEvent`'s branch; `TelegramAllocutio._handleActumStage`. Migrated: the **🔥 warm reaction** now fires off `actum.progressus` (`provisioning`+`'warm pod reused'`) in `_handleActumProgressus`; `BulletinManager._startStudio` (`/arm` Start, runs outside an actum trace) projects the provisioner's string callback through `coldStartProgressus`→`session.onProgressus` (the one place that string callback lands — no double-drive, since the arm session is never `register`ed with an actumId so any bus `actum.progressus` buffers unapplied); `liveRouter` (internal debug SSE) forwards `actum.progressus`. comfyrunner's bare-`downloading` (no preflight count) now records a `downloading` Progressus so the bulletin/SSE still show it; per-tick sampler `progress` is dropped (live-only by design §7; an `actum.progressus` per step would flood the Telegram render). `StageInfo` survives as the cold-start carrier on the `StudioStageCb` ring-internal callback (projected onto `Progressus.pod`). Tests migrated to the owned channel (PodSession/BulletinManager via `fakeStageToProgressus`, RunEventHub/runEvents/apiRouterStream to `actum.progressus`, Fake/Warm clients to the recorder seam); the legacy `emitStage`/`progressusToStage`/stage-frame tests deleted. **Build #6 complete.**
