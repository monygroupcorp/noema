...
6. 🟡 **Consumers (incremental; legacy `actum.stage` shim stays until 6e):** `StatusView`/bulletin/frontend read `Progressus`; derive `ActumExecutio` from phase durations.
   ...
   - 🟠 6e delete `progressusToStage` + `actum.stage` + `emitStage` + `PodSession.onStage`/`_pendingStages`/`BulletinManager.onStage`. **6e note:** `BulletinManager._startStudio` (the `/arm` Start path) drives `session.onStage(...)` DIRECTLY (not via the bus — it runs outside an actum trace, so no `actum.progressus` fires; safe from double-drive today), so 6e must migrate that call site to `onProgressus` (or a small direct method) when `onStage` is deleted.
