...
1. ✅ **Spine + feed bite** — SHIPPED 2026-06-21. `Editio` (`src/types/editio.ts`) + `Editionum`
   ...
   synchronous publish — the gate is injected as an interface; the **real CSAM/NCMEC gate is BUILT**
   (2026-07-02: fetch→hash→known-match→classifier→reject+CyberTipline report, fail-closed) but lives in the
   **PRIVATE compliance module** (ADR-0012 §49 — abuse surface, not published; `src/private/compliance`,
   injected at deploy). The public repo ships only the port + fail-closed stub. Two credentialed seams remain
   (classifier provider transport + live NCMEC submission) — see §10.
...
3. ✅ **Model publishing + custody preferences** — SHIPPED 2026-06-21. One registry-parameterized
   ...
   are PEER strategies — none baked into the adapter or spine (the explicit anti-overfit requirement). The
   **HF uploader is now built** (`HuggingFaceUploader` + `HfHttpTransport`, `src/crystal/HfUploader.ts`):
   streams weights to HF via the 3-step Git-LFS protocol inside the durable `PublicationWorker`, wired when
   `HF_TOKEN` is set (else projection-only). Orchestration is hermetically tested; the real HF HTTP is
   **live-unverified** (§10). **Civitai upload is PRECLUDED** — Civitai exposes no public write/POST API (uploads are
   GUI-locked), so a Civitai `RegistryUploader` is not buildable; the `civitaiRegistry` stays projection-only
   (it can LINK a user's existing Civitai model URL, never push one). HF is the only programmatic registry.
   The royalty payee (§5e) is the model's own `auctor`, unchanged by publish.
...
|---|---|---|---|
| `ModerationGate` port (`src/crystal/ModerationGate.ts`, PUBLIC) → real impl **PRIVATE** (ADR-0012 §49, `src/private/compliance`, injected at deploy) — **BUILT 2026-07-02** | The **real gate is now built** (in the private module, NOT this repo): fetch every media URL an artifact puts forth → SHA-256 exact match + perceptual near-match against a known-CSAM hash set → optional novel-material classifier → on a match, **reject + assemble a NCMEC CyberTipline report**. **Fail-closed** everywhere (unfetchable/unclassifiable item, or no detection capability ⇒ refuse). `src/index.ts` loads the private module via a guarded dynamic import when detection is configured (`CSAM_HASHSET_PATH` and/or a classifier); absent or unconfigured → `MODERATION_ALLOW_UNSCANNED` permits (dev only) or `denyModerationGate` refuses. The public repo ships only the port + `permissiveModerationGate`/`denyModerationGate` stubs. | **Two credentialed seams remain (LIVE-UNVERIFIED, like `HfHttpTransport`):** (a) a concrete **classifier provider transport** behind the `CsamClassifier` interface — none ships (provider-agnostic: PhotoDNA Cloud / Thorn Safer / Hive drops in when contracted); (b) the **live NCMEC CyberTipline submission** — `deferredNcmecReporter` assembles + preserves the report but does NOT submit (`submitted:false`) until a registered ESP account + legal sign-off exist. The known-CSAM **hash set** is provisioned out-of-band from a vetted provider (NOT a public dataset like OFAC's SDN), so it ships EMPTY. | Contract a detection provider + register the ESP, then wire the classifier transport + the live NCMEC submit. Still a go-live blocker **until a provider is wired** (an empty hash set + no classifier ⇒ fail-closed deny, which is safe but blocks the public feed). |
| `HfHttpTransport` — real HF HTTP, **LIVE-UNVERIFIED** (`src/crystal/HfUploader.ts`) | The HF **LFS uploader IS built** (2026-06-22): `HuggingFaceUploader` streams a model's weights to HF via the 3-step Git-LFS protocol (batch → presigned PUT → verify) + repo-create + commit, running inside the durable `PublicationWorker`. Its ORCHESTRATION is hermetically tested (fake transport); but `HfHttpTransport` — the real HF HTTP — **cannot be exercised without `HF_TOKEN` + network**, so the wire-level protocol (endpoint shapes, LFS batch/commit NDJSON, presigned PUT headers) is unverified against real HF. | A live test against real HF: publish a small LoRA end-to-end, confirm the repo + LFS object + commit land and the weights re-download. Fix protocol drift. **Civitai upload is precluded** (no public write API — GUI-locked; `civitaiRegistry` links existing models only, never uploads); real `retract` = repo deletion. | Before EXTERNAL model publishing is offered to users (HF path is code-complete but unproven). |
