...
- **TASK-005 · Save-as** — the flow card's "save as canon verb" / "save as my flow" buttons →
  register a derived `Modus` (owner-stamped) + `bindVerb`. Gated on the persistence layer (widen
  `Modus.auctor` to `{animaId}|{commitment}`, the owner-keyed store). See ADR-0003.
