...
Anonymity here is a property of how you fund. Pay by card and it's identified; fund from a fresh or shielded on-chain wallet and there's no identity behind the address — the strongest privacy available today. An unlinkable ZK bearer Bursa on top is coming soon (see below).
...
- Fund anonymously from a shielded wallet (unlinkable ZK Bursa coming soon)
...
Anonymity depends on how you fund. Fund from a fresh or shielded on-chain wallet and the address has no identity behind it — the strongest privacy available today, and it works right now. On top of that, an unlinkable ZK bearer Bursa — spends cryptographically unlinkable to what you funded — is coming soon: it unlocks after the trusted-setup ceremony. Direct-to-commitment deposits, where we never see the funding wallet, are also on the roadmap.
...
The ZK bearer Bursa is coming soon (it unlocks after the trusted-setup ceremony), so today every credit is a regular account-tied credit. When the Bursa ships, a Bursa credit will spend from a ZK bearer token whose spends are cryptographically unlinkable to how it was funded — functionally identical compute, same GPU, same models, just a different billing layer. Either way, how anonymous the funding itself is depends on your funding source: a shielded wallet reveals no identity; a card is identified.
...
You can fund anonymously today by depositing from a shielded or fresh on-chain wallet — no identity sits behind the address. Fully anonymous bearer-token spending (the `x-bursa-token` ZK Bursa) is coming soon, after the trusted-setup ceremony.
