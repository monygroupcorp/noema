...
export interface BulletinDeps {
   ...
   *  `hostUserId` (resolved to the studio's host AuctorKey by the adapter — so the studio is
   *  never host-less, ADR-0006). Returns the pod it parked (+ telemetry for the journal), or
   *  null if provisioning failed/unavailable. */
  startStudio?: (hostUserId: string, opts: { models: PendingModel[]; runtime?: string; warmMs?: number }, onStage?: (stage: string, info?: StageInfo) => void) => Promise<{ podId: string; gpuType?: string; costPerHr?: number; provisionMs?: number } | null>
