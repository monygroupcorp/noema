...
export const httpJsonFetcher: JsonFetcher = {
  async fetchJson(url: string): Promise<unknown> {
    ...
    if (!res.ok) throw new ModelImportError(`origin metadata fetch failed: ${url} → ${res.status}`)
    return res.json()
  },
}
...
async function resolveCivitai(url: string, json: JsonFetcher): Promise<ResolvedImport> {
  ...
  return {
    ...
    origin: {
      ...
      meta: { modelId: String(modelId), modelVersionId: String(version.id ?? versionId ?? ''), ...(author ? { author } : {}) },
    },
    downloadUrl,
    filename,
