...
export function createLiveRouter(secret?: string): Router {
  ...
  router.get('/live', (req, res) => {
    ...
    const onStage     = (data: unknown)   => sendEvent('actum.stage',    data)
    ...
    bus.on('actum.stage',    onStage    as Parameters<typeof bus.on>[1])
    ...
    req.on('close', () => {
      ...
      bus.off('actum.stage',    onStage    as Parameters<typeof bus.on>[1])
