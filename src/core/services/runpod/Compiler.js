...
class Compiler {
  constructor({ workflowTemplates, userPreferencesDb, logger, randomSeed } = {}) {
  ...
  async compile({ tool, inputs, accountContext } = {}) {
    if (tool && Array.isArray(tool.composedSteps) && tool.composedSteps.length > 0) {
      throw new CompilerError('COMPOSED_NOT_SUPPORTED', 'composed Tools not yet supported in Phase 1');
    }
    ...
    return this._compileAtomic(tool, inputs || {}, accountContext || null);
  }
...
if (require.main === module) {
  (async () => {
    ...
    // H. Composed throws
    {
      ...
      let caught = null;
      try {
        await c.compile({ tool: { ...tool, composedSteps: [{}] }, inputs: { prompt: 'a cat' }, accountContext: ctx });
      } catch (err) {
        caught = err;
      }
      if (!caught) failures.push('H: composed did not throw');
      else if (!(caught instanceof CompilerError)) failures.push(`H: expected CompilerError, got ${caught.constructor.name}`);
      else if (caught.code !== 'COMPOSED_NOT_SUPPORTED') failures.push(`H: expected code COMPOSED_NOT_SUPPORTED, got ${caught.code}`);
    }
    ...
    console.log('  - H composedSteps throws CompilerError COMPOSED_NOT_SUPPORTED');
  })().catch((err) => {
