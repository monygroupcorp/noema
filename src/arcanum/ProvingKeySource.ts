...
export function createProvingKeySource(opts: ProvingKeySourceOpts): ProvingKeySource {
  ...
  async function repoKeyIfItIsTheCeremonys(finalHash: string): Promise<ServedProvingKey | null> {
    ...
    return { origin: 'ceremony', hash: finalHash, path: repo.path }
  }
  ...
  async function fromCeremony(finalHash: string): Promise<ServedProvingKey> {
    ...
    if (!bytes) {
      ...
      if (fromImage) {
        ceremonyKey = fromImage
        return fromImage
      }
      const repo = await fromRepo()
      log.error('ceremony finalized but the final proving key is in neither custody nor the ' +
