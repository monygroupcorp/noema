...
export function createProvingKeySource(opts: ProvingKeySourceOpts): ProvingKeySource {
  ...
  // Every condition below holds until someone changes the deployment, and this source is
  // re-resolved on every GET /arcanum/config and every GET /arcanum/circuit/zkey — so an
  // unguarded log.error on one of them is a line per request. See `faultReporter`. The
  // keys are the status fault and three per final key a ceremony has published, so more
  // than one can stand at a time and none of them is forgotten by the next.
  const faults = faultReporter(log, opts.now)
