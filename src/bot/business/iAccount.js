const { lobby, rooms, STATES, startup, getBurned, getNextPeriodTime ,
    actionMap,
    prefixHandlers
...
const { verifyHash } = require('../core/users/verify.js')
const { signedOut } = require('../core/models/userKeyboards.js')
const { features } = require('../core/models/tokengatefeatures.js')
const {defaultUserData,validateUserData} = require('../core/users/defaultUserData.js')
const { getBalance } = require('../core/users/checkBalance')
const { getGroup } = require('../periphery/iGroup')
const { home } = require("../core/iMenu")
const { AnalyticsEvents } = require('../../db/models/analyticsEvents.js');
const { TutorialManager, CHECKPOINTS } = require('../core/iStart');
...
const { fetchUserCore, fetchFullUserData } = require('../../db/operations/userFetch.js');
const { writeNewUserDataMacro } = require('../../db/operations/newUser.js');
    ...
    if (!hasApiKey) {
        ...
        await editMessage({
            ...
            text: `🔑 API Key Management\n\n` +
                ...
                `1. Send requests to: ${API_URL}\n` +
                `2. Include your API key in the Authorization header:\n` +
                `   \`Authorization: Bearer ms2_YOUR_API_KEY\`\n\n` +
