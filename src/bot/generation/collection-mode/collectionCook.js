const { db, studio, lobby, flows, waiting } = require('../../core/core');
const { 
    getOrLoadCollection, 
    findExclusions, 
...
class CollectionCook {
    ...
    constructor() {
        ...
        this.collectionDB = db.studio;
        this.globalStatusDB = db.globalStatus;
    ...
    async resumeCooking(cookTask) {
        try {
            ...
            if (lobby[user]?.qoints) {
                ...
                const userEco = await db.userEconomy.findOne({ userId: user });
                if (userEco) {
    ...
    async reviewCooking(action, message, user) {
        try {
            ...
            const pieces = await db.studio.findMany({ collectionId });
    ...
    async handleReviewStart(action, message, user) {
        try {
            ...
            const piece = await db.studio.getNextPendingPiece(collectionId);
            if (!piece) {
    ...
    async handleReviewAction(action, message, user) {
        try {
            ...
            await db.studio.updatePieceStatus(collectionId, pieceId, isApprove ? 'approved' : 'rejected');
    ...
    async handleStats(action, message, user) {
        try {
            ...
            const collection = await getOrLoadCollection(user, collectionId);
            const stats = await db.studio.getCollectionStats(collectionId);
    ...
    async handlePromptShow(action, message, user) {
        try {
            ...
            const piece = await db.studio.getPieceById(pieceId);
            // Get the piece
            if (!piece) {
