const { db, studio, prefixHandlers, STATES, stateHandlers, lobby } = require('../../core/core'); 
...
class CollectionExport {
    ...
    async getExportableCollections(userId) {
        ...
        const collections = await db.studio.getCollectionsByUserId(userId);
        const exportable = {
    ...
    async handleExportSelectionImages(action, message, user) {
        ...
        try {
            ...
            for (const collectionId of selectedCollections) {
                ...
                const pieces = await db.studio.findMany({
                    collectionId,
        ...
        switch (platform) {
            case 'launchmynft':
                ...
                studio[user].launchMyNFT = {
                    ...
                    collections: await db.studio.findMany({
    ...
    try {
        ...
        if (studio[user]?.mustprovidepinnedaddress) {
            ...
            try {
                ...
                const collections = await db.studio.findMany({
