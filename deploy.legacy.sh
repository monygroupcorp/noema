...
if grep -q "✅ Collection.*Found.*completed generations" src/api/internal/cookApi.js; then
  ...
  echo "⚠️  This might indicate a git pull issue. Check git status." >> "${LOG_FILE}" 2>&1
fi

# Touch a file to ensure Docker sees the code has changed (forces COPY cache invalidation)
...
GIT_COMMIT=$(git rev-parse HEAD)
echo "📌 Building Docker image for git commit: ${GIT_COMMIT}" >> "${LOG_FILE}" 2>&1
...
# Build with cache - the .docker-build-trigger file ensures COPY layer is invalidated when code changes
