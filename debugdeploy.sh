...
log "=== StationThis DEBUG deployment started (container left running on failure) ==="
rotate_logs
...
if ! health_check_app; then
  log "Health check failed; collecting container logs (container will remain running)..."
  if docker ps -a --format '{{.Names}}' | grep -q "^${APP_CONTAINER}$"; then
    docker logs "${APP_CONTAINER}" | tail -n 200 >> "${LOG_FILE}" 2>&1 || true
    log "Container ${APP_CONTAINER} left running for investigation."
    log "Use 'docker logs -f ${APP_CONTAINER}' or 'docker exec -it ${APP_CONTAINER} /bin/bash' to inspect."
  else
    log "Container ${APP_CONTAINER} is not present; nothing to debug."
  fi
  unset PRIVATE_KEY
  disable_maintenance
  log "Exiting debug deploy early so you can investigate."
  exit 1
fi
unset PRIVATE_KEY
